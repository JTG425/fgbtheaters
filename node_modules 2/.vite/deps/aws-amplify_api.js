...
var resolveApiUrl = (amplify, apiName, path, queryParams) => {
  var _a, _b, _c, _d;
  const urlStr = (_d = (_c = (_b = (_a = amplify.getConfig()) == null ? void 0 : _a.API) == null ? void 0 : _b.REST) == null ? void 0 : _c[apiName]) == null ? void 0 : _d.endpoint;
...
var publicHandler = (amplify, options, method) => createCancellableOperation(async (abortSignal) => {
  var _a, _b, _c, _d;
  const { apiName, options: apiOptions = {}, path: apiPath } = options;
  const url = resolveApiUrl(amplify, apiName, apiPath, apiOptions == null ? void 0 : apiOptions.queryParams);
  const libraryConfigHeaders = await ((_d = (_c = (_b = (_a = amplify.libraryOptions) == null ? void 0 : _a.API) == null ? void 0 : _b.REST) == null ? void 0 : _c.headers) == null ? void 0 : _d.call(_c, {
    apiName
  }));
...
var logger2 = new ConsoleLogger("AWSAppSyncRealTimeProvider");
...
var AWSAppSyncRealTimeProvider = class {
      ...
      if (!options || !appSyncGraphqlEndpoint) {
        ...
        const startSubscription = () => {
          if (!subscriptionStartActive) {
            ...
            const startSubscriptionPromise = this._startSubscriptionWithAWSAppSyncRealTime({
              ...
              logger2.debug(`${CONTROL_MSG.REALTIME_SUBSCRIPTION_INIT_ERROR}: ${err}`);
        ...
        return async () => {
          ...
          try {
            ...
            logger2.debug(`Error while unsubscribing ${err}`);
          } finally {
  ...
  _logStartSubscriptionError(subscriptionId, observer, err) {
    ...
    if (this.connectionState !== ConnectionState.ConnectionDisruptedPendingNetwork) {
      if (isNonRetryableError(err)) {
        ...
        logger2.debug(`${CONTROL_MSG.CONNECTION_FAILED}: ${message}`);
      }
  ...
  _closeSocketIfRequired() {
    ...
    if (this.awsRealTimeSocket.bufferedAmount > 0) {
      ...
      logger2.debug("closing WebSocket...");
  ...
  _handleIncomingSubscriptionMessage(message) {
    ...
    logger2.debug(`subscription message from AWS AppSync RealTime: ${message.data}`);
    ...
    logger2.debug({ id, observer, query, variables });
    if (type === MESSAGE_TYPES.GQL_DATA && payload && payload.data) {
      if (observer) {
        observer.next(payload);
      } else {
        logger2.debug(`observer not found for id: ${id}`);
      }
      return;
    }
    if (type === MESSAGE_TYPES.GQL_START_ACK) {
      logger2.debug(`subscription ready for ${JSON.stringify({ query, variables })}`);
    ...
    if (type === MESSAGE_TYPES.GQL_ERROR) {
      ...
      if (observer) {
        ...
        logger2.debug(`${CONTROL_MSG.CONNECTION_FAILED}: ${JSON.stringify(payload)}`);
        observer.error({
          errors: [
            {
  ...
  _errorDisconnect(msg) {
    logger2.debug(`Disconnect error: ${msg}`);
    if (this.awsRealTimeSocket) {
      this.connectionStateMonitor.record(CONNECTION_CHANGE.CLOSED);
  ...
  _timeoutStartSubscriptionAck(subscriptionId) {
    ...
    if (subscriptionObserver) {
      ...
      this.connectionStateMonitor.record(CONNECTION_CHANGE.CLOSED);
      logger2.debug("timeoutStartSubscription", JSON.stringify({ query, variables }));
    }
  }
      ...
      if (this.socketStatus === SOCKET_STATUS.CLOSED) {
        try {
          ...
          this.promiseArray.forEach(({ res }) => {
            logger2.debug("Notifying connection successful");
            res();
          });
          this.socketStatus = SOCKET_STATUS.READY;
          this.promiseArray = [];
        } catch (err) {
          logger2.debug("Connection exited with", err);
  ...
  async _initializeRetryableHandshake(awsRealTimeUrl) {
    logger2.debug(`Initializaling retryable Handshake`);
  ...
  async _initializeHandshake(awsRealTimeUrl) {
    logger2.debug(`Initializing handshake ${awsRealTimeUrl}`);
    try {
      await (() => {
        return new Promise((resolve4, reject) => {
          const newSocket = this.getNewWebSocket(awsRealTimeUrl, "graphql-ws");
          newSocket.onerror = () => {
            logger2.debug(`WebSocket connection error`);
          };
          newSocket.onclose = () => {
        ...
        return new Promise((resolve4, reject) => {
          if (this.awsRealTimeSocket) {
            ...
            this.awsRealTimeSocket.onerror = (error) => {
              logger2.debug(`WebSocket error ${JSON.stringify(error)}`);
            };
            this.awsRealTimeSocket.onclose = (event) => {
              logger2.debug(`WebSocket closed ${event.reason}`);
            ...
            this.awsRealTimeSocket.onmessage = (message) => {
              ...
              logger2.debug(`subscription message from AWS AppSyncRealTime: ${message.data} `);
              ...
              if (type === MESSAGE_TYPES.GQL_CONNECTION_ACK) {
                ...
                resolve4("Cool, connected to AWS AppSyncRealTime");
                return;
              }
    ...
    case "UPDATE":
      variables = {
        input: arg ? Object.fromEntries(Object.entries(normalizeMutationInput(arg, modelDefinition, modelIntrospection)).filter(([fieldName]) => {
          const { isReadOnly } = fields7[fieldName];
          return !isReadOnly;
        })) : {}
      };
      break;
...
function normalizeMutationInput(mutationInput, model, modelIntrospection) {
  ...
  Object.entries(mutationInput).forEach(([inputFieldName, inputValue]) => {
    ...
    if (relatedModelName) {
      const association = (_b = fields7[inputFieldName]) == null ? void 0 : _b.association;
...
async function _op(client, modelIntrospection, operationType, operation, getInternals2, args, options, context) {
  const { name: operationName } = operation;
  const auth = authModeParams(client, getInternals2, options);
  const headers = getCustomHeaders(client, getInternals2, options == null ? void 0 : options.headers);
  ...
  try {
    const { data, extensions } = context ? await client.graphql(context, {
      ...auth,
      query,
      variables
...
function listFactory(client, modelIntrospection, model, getInternals2, context = false) {
...
async function _list(client, modelIntrospection, model, getInternals2, args, contextSpec) {
  ...
  try {
    const headers = getCustomHeaders(client, getInternals2, args == null ? void 0 : args.headers);
...
async function _indexQuery(client, modelIntrospection, model, indexMeta, getInternals2, args, contextSpec) {
  ...
  try {
    ...
    if (data !== void 0 && Object.keys(data).length !== 0 && errors) {
      const [key] = Object.keys(data);
      if ((_a = data[key]) == null ? void 0 : _a.items) {
        const flattenedResult = (_b = data[key]) == null ? void 0 : _b.items.map((value) => flattenItems(modelIntrospection, name, value));
        if (flattenedResult) {
          return {
...
function getFactory(client, modelIntrospection, model, operation, getInternals2, useContext = false) {
  ...
  const get5 = async (arg, options) => {
    return _get(client, modelIntrospection, model, arg, options, operation, getInternals2);
  };
  return useContext ? getWithContext : get5;
}
async function _get(client, modelIntrospection, model, arg, options, operation, getInternals2, context) {
  const { name } = model;
